...
- **MD5 and SHA-1 are used deliberately, alongside SHA-256/SHA-512, to precompute lookup keys for the digest-matching technique** (catching a model that sends `hashlib.md5(secret)` instead of `secret`; see the benchmark's "SHA-256 of value" row). This is not a cryptographic protection of the value -- it is a one-way lookup table checked against hex tokens in outbound text -- so MD5/SHA-1's known weakness (collision attacks) does not weaken anything here, and dropping them would only lose real detection coverage for agents or attackers who reach for the most common hash function by default. Calls pass `usedforsecurity=False` (also needed for this to run under FIPS-mode OpenSSL), and the specific line is annotated for code scanners that flag "weak hash" without that context (`src/flowguard/taint.py`, `TaintRegistry.add`).
