...
- Marked the MD5/SHA-1 calls used for digest-matching lookup keys with `usedforsecurity=False` (also required under FIPS-mode OpenSSL) and documented why they are not a weakened cryptographic protection, in response to a CodeQL "weak hash on sensitive data" alert -- see `docs/THREAT_MODEL.md#design-decisions-and-their-costs`.
